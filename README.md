...
- Запуск тестов в Xcode: меню Product → Test (или Cmd+U).
 - UI-тесты находятся в `DeadlineCountdownUITests` и проверяют ключевые сценарии интерфейса (создание дедлайна, навигация между экранами, базовые взаимодействия). UI-тесты также можно запускать через Product → Test или `xcodebuild test` с указанием подходящего симулятора.
